...
let ultimoResultadoDia = null;      // para salvar e editar
  ...
  const html = `
    <article class="escala-documento">
      ...
      <section class="escala-section">
        <h3>
          <span>🍽 Almoço</span>
          <button type="button" class="editar-bloco" data-bloco="almoco">Editar</button>
        </h3>
      ...
      <section class="escala-section">
        <h3>
          <span>☕ Lanche</span>
          <button type="button" class="editar-bloco" data-bloco="lanche">Editar</button>
        </h3>
      ...
      <section class="escala-section">
        <h3>
          <span>🧺 Aparadores & Setores</span>
          <button type="button" class="editar-bloco" data-bloco="aparadores">Editar</button>
        </h3>
        <table class="escala-table">
          <thead>
            <tr>
      ...
      <section class="escala-section">
        <h3>
          <span>🍹 Bar</span>
          <button type="button" class="editar-bloco" data-bloco="bar">Editar</button>
        </h3>
        <table class="escala-table">
          <thead>
            <tr>
...
btnGerarDia.addEventListener("click", () => {
  ...
  ultimoResultadoDia = escala; // guarda para salvar no histórico e editar
...
// --------- EDIÇÃO DAS FUNÇÕES (MODAL) ---------
const modalEdicaoEl = document.getElementById("modal-edicao");
const modalTituloEl = document.getElementById("modal-titulo");
const modalListaEl = document.getElementById("modal-lista");
const modalSalvarBtn = document.getElementById("modal-salvar");
const modalCancelarBtn = document.getElementById("modal-cancelar");

let blocoAtualEdicao = null;

// Abre modal ao clicar nos botões "Editar"
document.addEventListener("click", (e) => {
  const btn = e.target;
  if (btn.classList.contains("editar-bloco")) {
    if (!ultimoResultadoDia || !ultimoResultadoDia.presentes) return;
    blocoAtualEdicao = btn.dataset.bloco;
    abrirModalEdicao();
  }
});

function labelBloco(bloco) {
  switch (bloco) {
    case "almoco": return "Almoço";
    case "lanche": return "Lanche";
    case "aparadores": return "Aparadores";
    case "bar": return "Bar";
    default: return "";
  }
}
...
function abrirModalEdicao() {
  ...
  modalTituloEl.textContent = "Editar " + labelBloco(blocoAtualEdicao);

  ultimoResultadoDia.presentes.forEach((p) => {
    ...
    label.append(" " + p.nome);
    modalListaEl.appendChild(label);
  });

  modalEdicaoEl.classList.remove("hidden");
}

modalCancelarBtn.addEventListener("click", () => {
  modalEdicaoEl.classList.add("hidden");
});

modalSalvarBtn.addEventListener("click", () => {
  if (!blocoAtualEdicao || !ultimoResultadoDia) {
    modalEdicaoEl.classList.add("hidden");
    return;
  }

  const checks = modalListaEl.querySelectorAll("input[type='checkbox']:checked");
  const idsSelecionados = Array.from(checks).map((chk) => Number(chk.value));
  const selecionados = ultimoResultadoDia.presentes.filter((p) =>
    idsSelecionados.includes(p.id)
  );

  const roles = ultimoResultadoDia.roles;

  if (blocoAtualEdicao === "almoco") {
    const metade = Math.ceil(selecionados.length / 2);
    roles.almocoTurma1 = selecionados.slice(0, metade);
    roles.almocoTurma2 = selecionados.slice(metade);
  }

  if (blocoAtualEdicao === "lanche") {
    const t1 = Math.ceil(selecionados.length / 3);
    const t2 = Math.ceil((selecionados.length - t1) / 2);
    roles.lancheTurma1 = selecionados.slice(0, t1);
    roles.lancheTurma2 = selecionados.slice(t1, t1 + t2);
    roles.lancheTurma3 = selecionados.slice(t1 + t2);
  }

  if (blocoAtualEdicao === "aparadores") {
    roles.aparadores = [
      selecionados[0] || null,
      selecionados[1] || null,
      selecionados[2] || null
  ...
  if (blocoAtualEdicao === "bar") {
    roles.bar1 = selecionados[0] || null;
    roles.bar2 = selecionados[1] || null;
  }
